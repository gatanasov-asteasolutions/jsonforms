...
const evaluateCondition$1 = (data, condition, path, ajv) => {
    if (isAndCondition(condition)) {
        return condition.conditions.reduce((acc, cur) => acc && evaluateCondition$1(data, cur, path, ajv), true);
    }
    else if (isOrCondition(condition)) {
        return condition.conditions.reduce((acc, cur) => acc || evaluateCondition$1(data, cur, path, ajv), false);
    }
...
const checkDataCondition = (propertyCondition, property, data) => {
    if (has(propertyCondition, 'const')) {
        ...
        return (has(data, property) &&
            get(propertyCondition, 'enum').find((value) => isEqual(value, data[property])) !== undefined);
    }
...
const checkPropertyCondition = (propertiesCondition, property, data) => {
...
const evaluateCondition = (schema, data) => {
    if (has(schema, 'allOf')) {
        return all((subschema) => evaluateCondition(subschema, data), get(schema, 'allOf'));
    }
    if (has(schema, 'anyOf')) {
        return any((subschema) => evaluateCondition(subschema, data), get(schema, 'anyOf'));
    }
    if (has(schema, 'oneOf')) {
    ...
    const requiredCondition = all((property) => has(data, property), requiredProperties);
    if (has(schema, 'properties')) {
        const propertiesCondition = get(schema, 'properties');
        const valueCondition = all((property) => checkPropertyCondition(propertiesCondition, property, data), Object.keys(propertiesCondition));
        return requiredCondition && valueCondition;
    }
    return requiredCondition;
};
const extractRequired = (schema, segment, prevSegments) => {
    let i = 0;
    ...
    return (has(currentSchema, 'required') &&
        get(currentSchema, 'required').includes(segment));
};
const checkRequiredInIf = (schema, segment, prevSegments, data) => {
    ...
    return ((has(schema, 'then') &&
        ...
        (allOfInThen &&
            condition &&
            conditionallyRequired(get(schema, 'then'), segment, prevSegments, data)) ||
        (allOfInElse &&
            !condition &&
            conditionallyRequired(get(schema, 'else'), segment, prevSegments, data)));
};
const conditionallyRequired = (schema, segment, prevSegments, data) => {
    const nestedAllOfSchema = get(schema, 'allOf');
    return any((subschema) => {
        return ((has(subschema, 'if') &&
